    ...
    <section id="home" className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 sm:py-40">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="max-w-3xl">
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-xs sm:text-sm font-semibold tracking-[0.2em] text-amber-400 uppercase mb-4">
            {hero.eyebrow}
          </motion.p>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8 }} className="text-4xl sm:text-5xl lg:text-6xl font-heading font-bold text-white leading-[1.1] mb-6">
            {hero.heading}
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.8 }} className="text-xl sm:text-2xl text-white/80 leading-relaxed mb-10 max-w-2xl">
            {hero.subheading}
          </motion.p>
          ...
          <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} onClick={() => handleScroll("#stay")} className="text-base font-medium text-white/60 hover:text-white transition-colors flex items-center gap-1.5">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.8 }} className="hidden sm:flex gap-3 mt-2">
          <div className="bg-white/10 backdrop-blur-xl rounded-2xl px-5 py-3 shadow-lg shadow-black/20 border border-white/20">
            <div className="text-sm font-semibold text-white">{hero.roomLabel}</div>
            <div className="text-sm text-white/70">{hero.occupancyLabel}</div>
          </div>
          <div className="inline-flex items-center gap-2 px-4 py-3 bg-white/10 backdrop-blur-xl rounded-2xl shadow-lg shadow-black/20 border border-white/20">
            <div className="text-xs font-medium text-white/70">{hero.detailBadge}</div>
          </div>
        </motion.div>
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="absolute bottom-8 left-1/2 -translate-x-1/2">
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2 }} className="w-6 h-10 rounded-full border-2 border-white/30 flex items-start justify-center pt-2">
